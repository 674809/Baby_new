...
public class DBHelper extends SQLiteOpenHelper {
	...
	public static final String DB_NAME = "baby"; // DB name
	...
	public void onCreate(SQLiteDatabase db) {
		// 排出
		String sql = "create table if not exists expend(id integer primary key,year text,"+
                "month text,time LONG, shi text,niao text,other text,total text)";
		db.execSQL(sql);
		// 吃奶
		String sql1 = "create table if not exists income(id integer primary key,year text," +
                "month text,time LONG,mamilk text,milk text,other text," + "total text)";
		db.execSQL(sql1);
	}
